...
### 10.1 derived task 只读修正

- derived task 不再显示任务状态修改按钮（todo/doing/done/skipped/overdue/conflict）。
- derived task 不再显示审核状态按钮（pending/confirmed/ignored/conflict）。
- derived task 不再显示锁定/解锁按钮。
- derived task 显示只读提示："雷达推导任务，仅用于当前章提醒；需要持久化请在人工微调区新增当前章任务。"
- persisted task（非 legacy、非 derived）保留原有编辑能力。
- legacy task 保持只读。

### 10.2 "本章必须处理"分组 fallback 修正

- 修正前：`{groups.focusTasks || groups.focusThreads || []}` — 空数组 `[]` 为 truthy，不会 fallback。
- 修正后：`(groups.focusTasks || []).length ? (groups.focusTasks || []) : (groups.focusThreads || [])` — 空数组时正确 fallback 到 focusThreads。

### 10.3 伏笔相关文案继续中文化

- `ChapterSelect` 空选项：`None` → `不选择`。
- 保存伏笔线程：英文 → `伏笔修改已保存。` / `伏笔已保存为待确认记录，需要确认后才能锁定。`
- 保存生命周期事件：英文 → `伏笔生命周期事件已保存为待确认记录。`
- 保存当前章任务：英文 → `当前章伏笔任务已保存为待确认记录。`
- 选择错误提示中文：`请填写伏笔标题。` / `请先选择伏笔。` / `请先选择伏笔和任务章节。`

### 10.4 Phase 7 状态修正

- `loadContinuity` catch 错误文案：`Phase 7.2 连续性数据加载失败` → `Phase 7 连续性数据加载失败`。
- Phase 7 阶段状态：7.2 从"本轮实现"改为"已完成"，7.3 从"本轮实现"改为"已完成"。
- Phase 7 阶段样式：`已完成` 和 `本轮实现` 均显示绿色边框，避免 7.2/7.3 看起来未完成。
- Phase 7.4/7.5 保持"待开发"。

### 10.5 本章必须处理分组

修正后"本章必须处理"分组的渲染规则：
1. 如果 focusTasks 有数据，使用 focusTasks 展示任务卡片。
2. 如果 focusTasks 为空但 focusThreads 有数据，使用 focusThreads 展示伏笔线程卡片。
3. 如果 items 中有 taskType 字段，渲染 `ForeshadowingTaskCard`。
4. 否则渲染 `ForeshadowingThreadCard`。
5. 无数据时显示"暂无数据。"。
