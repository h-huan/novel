...
早期阶段中未完成的 Tab 保持真实边界，不用假数据伪装完成。
...
- subtitle 已随 Phase 7.5 升级为写作前检查与写作后更新闭环说明。
...
- renderFocus 调用已显式传入 relatedWorldTasks、relatedWorldRules、relatedTimelineTasks、relatedTimelineEvents、legacyTimelineEvents。
- 当前章焦点 Tab 的世界观 / 时间线空态已改为真实空态，不再暗示功能未完成。
...
- 总览 Tab 已展示 8 项 Phase 7.4 专项统计，并在“下一个创作动作”加入世界观 / 时间线提醒。
- 时间线 Tab 的 JSX 补丁残留声明已清理。
- 世界观 Tab 可编辑规则下拉改为基于 allRules 内部过滤，不再依赖未传入的 input.editableWorldRules。
- 时间线 Tab 可编辑事件下拉改为基于 allEvents 内部过滤，不再依赖未传入的 input.allEvents。
- 本轮未新增数据库迁移，未修改 022 迁移，未开发 Phase 7.5，未开发写作前检查闭环，未开发写作后更新闭环。

## 21. Phase 7.5 状态更新

Phase 7.5 已开始并补齐第一版写作前检查与写作后更新闭环。

本轮完成：

- 在 continuity 模块内新增写作前检查 API：`GET /precheck` 和 `POST /precheck/run`。
- 在 continuity 模块内新增写作后更新 API：`GET /postupdate`、`POST /postupdate/run`、confirm、ignore、conflict 操作。
- 写作前检查围绕 currentFocusChapter 输出 blocker、warning、pass、suggestion 分组，并给出是否建议开始写作。
- 写作前检查覆盖章节、大纲、人物、关系、伏笔、世界观、时间线、locked 和 pending 待确认项。
- 写作后更新围绕当前章正文输出人物状态、人物关系、伏笔、世界观、时间线和冲突建议。
- 当前章无正文或 locked 时不会假装成功，只输出 conflict / lockedConflict 建议。
- 用户确认写作后建议时只生成 `state_items` pending 项，不直接 confirmed，不覆盖 locked / confirmed 设定。
- precheck / postupdate Tab 已从空态入口升级为真实功能区。
