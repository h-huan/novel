...
## 输出格式
输出合法JSON，不要markdown包裹：
{"volumes":[{"volumeNumber":1,"title":"卷标题","theme":"主题","chapters":null,"chapterCountReason":"本卷为什么需要这些章节","outline":"本卷详细大纲","keyEvents":["关键事件"],"foreshadowings":[{"item":"伏笔内容","setupChapter":null,"revealChapter":null}]}]}
输出时按实际情节替换 null；伏笔位置只能在章节结构确定后填写。`,
      versions: [
...
## 执行要求
1. **按任务生成概要**：只生成调用方要求的章节范围；若未给范围，则按当前可可靠规划的连续故事阶段生成，不自行套用固定章数
...
{"chapters":[{"chapterNumber":1,"title":"章节标题","summary":"本章概要（100字）","keyEvents":["事件1"],"characters":["角色1"],"foreshadowings":[{"item":"伏笔","type":"埋设"}]}]}`,
    ...
    this.registerTemplate({
      ...
      category: 'long-novel-outline',
      version: '2.0.0',
      description: '已停用：单次调用无法承载完整长篇。项目创建改用地基、逐卷和逐批章纲流程。',
      content: `你是一名资深网络小说创作策划，擅长为长篇小说设计完整的大纲体系。

## 参考标准
- 两百万字网文创作全流程：遵守其阶段、资料字段和门禁；示例卷章数量不是约束
...
### 四、卷结构（卷数按不可合并的叙事阶段动态产生）
每卷包含：
- volumeNumber: 卷号
- title: 卷标题
- theme: 卷主题（如"初入江湖"、"真相浮现"）
- outline: 卷大纲（300字）
- keyEvents: 关键事件（数量由本卷因果链实际需要决定）
...
14. **volumePlan**：初步卷规划。卷数由故事结构决定；每卷必须说明主题、阶段目标、核心冲突、人物变化、卷末高潮，以及为何需要独立成卷。
...
1. **geography**：世界地理（数组，每个地点含 name+description，数量由实际舞台需要决定）
...
6. **history**：重要历史事件（数组，每个事件含 date+event，数量由因果链需要决定）
7. **factions**：主要势力/派系（数组，每个势力含 name+description+coreGoal+leader，数量由冲突结构需要决定）
...

{
  ...
  "skeletonVolumes": [
    {"volumeNumber":1, "title":"第一卷 ...", "theme":"本卷主题", "estimatedChapters":null, "chapterCountReason":"根据本卷事件链和节奏说明章数理由", "description":"本卷核心目标"}
  ]
}
...
## 执行要求
请根据主线任务、对抗关系和人物弧实际需要生成主要人物；每个角色都必须承担不可替代的叙事功能，并包含以下维度：
...
### 2. 分卷规划（动态决定卷数）
基于主线阶段、人物弧线、冲突升级、世界展开和阅读节奏划分。每卷都要有独立主题目标、阶段冲突与阶段高潮；不得使用预设卷数区间或平均分章。
...
输出合法JSON，不要markdown包裹：
{
  ...
  "pacing": {
    "setup": "铺垫阶段对应的实际卷",
    "development": "发展阶段对应的实际卷",
    "climax": "高潮阶段对应的实际卷",
    "resolution": "收尾阶段对应的实际卷"
  }
}
...
为初始大纲中的每一卷，生成：
1. **卷标题、核心主题**
2. **章节规划**（根据本卷事件链、冲突密度和节奏动态分配，不设固定区间）
   - 章节号、章节标题
   - 核心事件（本章主要发生什么）
   - 出场人物
   - 伏笔埋设/回收
...
### 1. 伏笔清单
根据主线、人物弧、世界规则和预定回收效果规划必要伏笔；数量不预设，每个伏笔包含：
...
### 2. 伏笔分类
- **主线伏笔**：与核心冲突直接相关，数量按主线因果链决定
- **人物伏笔**：与角色背景/动机相关，数量按人物弧决定
- **世界观伏笔**：与世界设定/历史真相相关，数量按规则揭示需要决定
- **情感伏笔**：与人物关系/情感变化相关，数量按关系转折决定
...
### 4. 伏笔密度控制
- **建立期**：只埋设当前阶段确有承载能力的伏笔，避免清单式堆叠
- **升级期**：按冲突与认知推进强化伏笔，不按章平均投放
- **回收期**：按证据、人物选择和情绪条件成熟度回收，避免机械集中回收
