...
export class MapPointService {
  constructor(private readonly repo: MapPointRepository, private readonly databaseService: DatabaseService, @Optional() private readonly stateItemService?: StateItemService) {
    const originalConsistency = this.checkConsistency.bind(this);
    (this as any).getWritingSummary = (projectId: string, id: string) => {
    ...
    (this as any).checkConsistency = (projectId: string, content: string) => { const result = originalConsistency(projectId, content); for (const point of this.findByProjectId(projectId)) { const profile = this.getProfile(projectId, point.id).profile; if (profile.owner_force && /(?:控制|占领|接管|统治|驻守|封锁)/.test(content) && !content.includes(profile.owner_force)) result.issues.push({ locationId: point.id, locationName: point.name, issueType: 'owner_force', evidence: profile.owner_force, reason: '正文可能写错地点控制势力', suggestion: `回扣地点控制势力：${profile.owner_force}`, severity: 'medium' }); } result.passed = result.issues.length === 0; result.score = Math.max(0, 100 - result.issues.length * 10); return result; };
  }
